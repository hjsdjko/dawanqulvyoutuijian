...
//                            jingdianOrderEntity.setJingdianOrderTime(new Date(data.get(0)));          //预定时间 要改的
//                            jingdianOrderEntity.setJingdianOrderNumber(Integer.valueOf(data.get(0)));   //预定人数 要改的
//                            jingdianOrderEntity.setCreateTime(date);//时间
    ...
    public R add(@RequestBody JingdianOrderEntity jingdianOrder, HttpServletRequest request) {
        ...
        if (jingdianEntity == null) {
            return R.error(511, "查不到该旅游景点");
        }
        if(yonghuEntity.getNewMoney()< (jingdianEntity.getJingdianMoney()*jingdianOrder.getJingdianOrderNumber())){
            return R.error("余额不足请充值");
        }
        yonghuEntity.setNewMoney(yonghuEntity.getNewMoney() - (jingdianEntity.getJingdianMoney()*jingdianOrder.getJingdianOrderNumber()));
